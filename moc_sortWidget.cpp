/****************************************************************************
** Meta object code from reading C++ file 'sortWidget.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.5.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "sortDemo/inc/sortWidget.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'sortWidget.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.5.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_sortWidget_t {
    QByteArrayData data[10];
    char stringdata0[96];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_sortWidget_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_sortWidget_t qt_meta_stringdata_sortWidget = {
    {
QT_MOC_LITERAL(0, 0, 10), // "sortWidget"
QT_MOC_LITERAL(1, 11, 4), // "step"
QT_MOC_LITERAL(2, 16, 0), // ""
QT_MOC_LITERAL(3, 17, 10), // "createSort"
QT_MOC_LITERAL(4, 28, 11), // "destroySort"
QT_MOC_LITERAL(5, 40, 8), // "updateUi"
QT_MOC_LITERAL(6, 49, 6), // "_swaps"
QT_MOC_LITERAL(7, 56, 9), // "_compares"
QT_MOC_LITERAL(8, 66, 14), // "reshuffleArray"
QT_MOC_LITERAL(9, 81, 14) // "toggleAutoStep"

    },
    "sortWidget\0step\0\0createSort\0destroySort\0"
    "updateUi\0_swaps\0_compares\0reshuffleArray\0"
    "toggleAutoStep"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_sortWidget[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   44,    2, 0x08 /* Private */,
       3,    0,   45,    2, 0x08 /* Private */,
       4,    0,   46,    2, 0x08 /* Private */,
       5,    2,   47,    2, 0x08 /* Private */,
       8,    0,   52,    2, 0x08 /* Private */,
       9,    0,   53,    2, 0x08 /* Private */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Int, QMetaType::Int,    6,    7,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void sortWidget::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        sortWidget *_t = static_cast<sortWidget *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->step(); break;
        case 1: _t->createSort(); break;
        case 2: _t->destroySort(); break;
        case 3: _t->updateUi((*reinterpret_cast< const int(*)>(_a[1])),(*reinterpret_cast< const int(*)>(_a[2]))); break;
        case 4: _t->reshuffleArray(); break;
        case 5: _t->toggleAutoStep(); break;
        default: ;
        }
    }
}

const QMetaObject sortWidget::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_sortWidget.data,
      qt_meta_data_sortWidget,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *sortWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *sortWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_sortWidget.stringdata0))
        return static_cast<void*>(const_cast< sortWidget*>(this));
    return QWidget::qt_metacast(_clname);
}

int sortWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
